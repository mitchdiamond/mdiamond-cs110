...
def pressure_gain_from_water_height(height):
    #Needed to add a constant for gravity and implement formula.
...
def pressure_loss_from_pipe(pipe_diameter, pipe_length, friction_factor, fluid_velocity):
    ...
    #Needed to adjust denominator, missing zero.
    denominator = 2000 * pipe_diameter
    return numerator / denominator

def pressure_loss_from_fittings(fluid_velocity, quantity_fittings):
    #Adjusted multiply by two to power of two.
    numerator = (-0.04 * WATER_DENSITY * (fluid_velocity ** 2) * quantity_fittings)
    return numerator / 2000

def reynolds_number(hydraulic_diameter, fluid_velocity):
      rn = (WATER_DENSITY * hydraulic_diameter * fluid_velocity) / VISCOSITY
      return rn 
